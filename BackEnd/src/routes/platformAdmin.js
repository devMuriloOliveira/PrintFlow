...
import { publishTenantSupportEvent } from '../services/supportRealtime.js'
...
export const handlePlatformAuditMessageCreate = async (req, res, requestId) => { const user = await requirePlatformAdmin(req, res); if (user) { const payload = await readJsonBody(req); const result = await addPlatformAuditMessage(user, requestId, payload); if (payload.visibility !== 'internal') publishTenantSupportEvent({ tenantId: result.tenant_id, requesterId: result.requested_by, requestId, type: 'message' }); await writePlatformAudit(req, user, { action: payload.visibility === 'internal' ? 'platform.support.internal_note_added' : 'platform.support.message_sent', targetResource: 'support_request', targetResourceId: requestId }); return sendJson(res, 201, {}) } }
...
export const handlePlatformAuditChatClose = async (req, res, requestId) => { const user = await requirePlatformAdmin(req, res); if (user) { const result = await closePlatformAuditChat(user, requestId); publishTenantSupportEvent({ tenantId: result.tenant_id, requesterId: result.requested_by, requestId, type: 'closed' }); await writePlatformAudit(req, user, { action: 'platform.support.chat_closed', targetTenantId: result.tenant_id, targetResource: 'support_request_chat', targetResourceId: requestId, details: { openedAt: result.chat_opened_at, closedAt: result.chat_closed_at } }); return sendJson(res, 200, result) } }
