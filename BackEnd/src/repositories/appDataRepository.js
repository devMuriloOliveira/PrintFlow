...
const readPrintJobs = async (client, tenantId) => {
  const result = await client.query(`
    select
      ...
      j.cancelled_at,
       ...
       j.slicing_recorded_at,
      j.estimated_print_seconds,
      j.estimated_filament_grams,
      j.estimated_filament_millimeters,
      j.actual_print_seconds,
      j.actual_filament_grams,
      j.actual_filament_millimeters,
      j.actual_material_cost,
      j.actual_energy_cost,
  ...
  return result.rows.map((row) => ({
    ...
    slicerProfileId: row.slicer_profile_id || '',
    slicerProfileVersion: row.slicer_profile_version || '',
    slicingArtifactStorageKey: row.slicing_artifact_storage_key || '',
    slicingArtifactName: row.slicing_artifact_name || '',
    slicingArtifactFormat: row.slicing_artifact_format || '',
    slicingArtifactSha256: row.slicing_artifact_sha256 || '',
    slicingArtifactSizeBytes: row.slicing_artifact_size_bytes == null ? null : Number(row.slicing_artifact_size_bytes),
    slicingRecordedAt: row.slicing_recorded_at || null,
    estimatedPrintSeconds: row.estimated_print_seconds == null ? null : Number(row.estimated_print_seconds),
    estimatedFilamentGrams: row.estimated_filament_grams == null ? null : Number(row.estimated_filament_grams),
    estimatedFilamentMillimeters: row.estimated_filament_millimeters == null ? null : Number(row.estimated_filament_millimeters),
    actualPrintSeconds: row.actual_print_seconds == null ? null : Number(row.actual_print_seconds),
    actualFilamentGrams: row.actual_filament_grams == null ? null : Number(row.actual_filament_grams),
    actualFilamentMillimeters: row.actual_filament_millimeters == null ? null : Number(row.actual_filament_millimeters),
    actualMaterialCost: row.actual_material_cost == null ? null : Number(row.actual_material_cost),
    actualEnergyCost: row.actual_energy_cost == null ? null : Number(row.actual_energy_cost),
