      ...
      <section class="settings-panel">
        ...
        <div v-else-if="active === 'Backup e Dados'" class="settings-security-card">
          ...
          <template v-else>
            ...
            <div v-if="auditRequests.length" class="table-scroll" style="margin-top:16px"><table class="data-table"><thead><tr><th>Protocolo</th><th>Status</th><th>Solicitada em</th></tr></thead><tbody><tr v-for="request in auditRequests" :key="request.id"><td>{{ request.id }}</td><td>{{ request.status }}</td><td>{{ new Date(request.createdAt).toLocaleString('pt-BR') }}</td></tr></tbody></table></div>
          </template>
          <hr style="border:0;border-top:1px solid var(--line);margin:24px 0">
