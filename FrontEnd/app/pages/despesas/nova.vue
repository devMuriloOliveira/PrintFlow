  ...
  <div>
    ...
    <div class="split-layout" style="grid-template-columns:minmax(0,1fr) 330px">
      <form @submit.prevent="save(false)">
        <div class="form-card">
          <h2 class="form-card__title"><UiIcon name="receipt" />1. Informações da Despesa</h2>
          <div class="form-grid">
        ...
        <div class="form-card">
          ...
          <div class="form-grid">
            <div class="field col-4"><label>Essa despesa é recorrente?</label><div class="switch-row"><span>{{form.recurring ? 'Sim' : 'Não'}}</span><button type="button" class="switch" :class="{active:form.recurring}" @click="form.recurring=!form.recurring" /></div></div>
        ...
        <div class="form-card">
          ...
          <div class="field"><label>Observações</label><textarea v-model="form.notes" placeholder="Detalhes internos sobre essa despesa" /></div>
        </div>
        <div class="form-actions"><button class="btn" type="button" @click="cancel">Cancelar</button><button v-if="!isEditing" class="btn" type="button" :disabled="saving" @click="save(true)">Salvar e adicionar outra</button><button class="btn btn--primary" type="submit" :disabled="saving">{{ saving ? 'Salvando...' : isEditing ? 'Salvar Alterações' : 'Salvar Despesa' }}</button></div>
      </form>
      <aside class="detail-card">
        ...
        <div class="detail-card__body">
          <div class="detail-list"><div class="detail-list__row"><span>Categoria</span><strong>{{form.category}}</strong></div><div class="detail-list__row"><span>Fornecedor</span><strong>{{form.supplier || 'Não informado'}}</strong></div><div class="detail-list__row"><span>Pagamento</span><strong>{{form.payment}}</strong></div><div class="detail-list__row"><span>Data</span><strong>{{form.date || '-'}}</strong></div><div class="detail-list__row"><span>Recorrência</span><strong>{{recurrence}}</strong></div></div>
