...
const handlePrintFileSelect = (event: Event) => {
  ...
  if (!allowedPrintFileFormats.has(format)) {
    ...
    input.value = ''
    notify('Formato nao permitido. Use 3MF, G-code ou BGCODE para impressao automatica.', 'info')
    return
  }
...
const validate = () => {
  ...
  if (!form.printFileName.trim()) errors.printFileName = 'Informe o arquivo de impressao.'
