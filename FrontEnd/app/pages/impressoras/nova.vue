...
const { printers, filaments } = useAppData()
const { notify } = useUi()
...
const save = (again = false) => {
  if (!validate()) return
  printers.value.unshift({ name: form.name, code: form.code || nextCode.value, maker: form.maker, model: form.model, acquired: form.acquired || '-', power: form.power, hours: form.hours, status: form.status === 'Imprimindo' ? 'Em Impressao' : form.status === 'Manutencao' ? 'Em Manutencao' : form.status, maintenance: form.maintenance || '-', serial: form.serial || '-' })
  ...
  <div>
    ...
    <div class="split-layout" style="grid-template-columns:minmax(0,1fr) 340px">
      <form @submit.prevent="save(false)">
        ...
        <div class="form-actions"><button class="btn" type="button" @click="cancel">Cancelar</button><button class="btn" type="button" @click="save(true)">Salvar e adicionar outra</button><button class="btn btn--primary" type="submit">Salvar Impressora</button></div>
      </form>
