...
const { printers, deleteItem } = useAppData()
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="(p,i) in printers" :key="p.id || p.code" :class="{selected:i===selectedIndex}" @click="selectedIndex=i">
                  ...
                  <td><div class="table-product table-product--editable"><span class="product-thumb"><UiIcon name="printer" :size="30" /></span><div><strong>{{p.name}}</strong><small>{{p.code}}</small></div><button class="row-action row-action--edit" title="Editar impressora" @click.stop="editPrinter(p)"><UiIcon name="edit" :size="15" /></button></div></td>
                  <td>{{p.maker}}</td>
                  <td>{{p.model}}</td>
                  <td>{{p.acquired}}</td>
                  <td>{{p.power}} W</td>
