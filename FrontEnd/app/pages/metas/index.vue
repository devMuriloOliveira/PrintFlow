...
<template>
  <div>
    ...
    <div class="metrics-grid metrics-grid--4"><MetricCard label="Metas Ativas" :value="formatNumber(metrics.activeGoals.value)" icon="target" note="Dados do banco" :points="goals.map(goal => goal.status !== 'Concluida' ? 1 : 0)" /><MetricCard label="Progresso Médio" :value="metrics.percent(metrics.goalsProgress.value)" icon="chart" note="Dados do banco" color="green" :points="goals.map(goal => goal.target ? Math.min(goal.current / goal.target * 100, 100) : 0)" /><MetricCard label="Metas Concluídas" :value="formatNumber(metrics.completedGoals.value)" icon="check" note="Dados do banco" color="purple" :points="goals.map(goal => goal.target > 0 && goal.current >= goal.target ? 1 : 0)" /><MetricCard label="Próxima Conclusão" :value="metrics.nextGoal.value?.name || '-'" icon="trend" :change="metrics.nextGoal.value ? `Faltam ${formatCurrency(metrics.nextGoal.value.target - metrics.nextGoal.value.current)}` : ''" color="orange" :points="goals.map(goal => Math.max(0, Number(goal.target || 0) - Number(goal.current || 0)))" /></div>
