...
<template>
  <div>
    <PageHeader title="Metas" subtitle="Defina objetivos e acompanhe a evolucao do seu negocio."><NuxtLink class="btn btn--primary" to="/metas/nova"><UiIcon name="plus" />Nova Meta</NuxtLink></PageHeader>
    <div class="metrics-grid metrics-grid--4"><MetricCard label="Metas Ativas" :value="formatNumber(metrics.activeGoals.value)" icon="target" note="Dados do banco" /><MetricCard label="Progresso Medio" :value="metrics.percent(metrics.goalsProgress.value)" icon="chart" note="Dados do banco" color="green" /><MetricCard label="Metas Concluidas" :value="formatNumber(metrics.completedGoals.value)" icon="check" note="Dados do banco" color="purple" /><MetricCard label="Proxima Conclusao" :value="metrics.nextGoal.value?.name || '-'" icon="trend" :change="metrics.nextGoal.value ? `Faltam ${formatCurrency(metrics.nextGoal.value.target - metrics.nextGoal.value.current)}` : ''" color="orange" /></div>
    <div class="dashboard-grid" style="grid-template-columns:repeat(2,1fr)">
      <PanelCard v-for="g in goals" :key="g.id || g.name">
        <div style="display:flex;gap:12px;align-items:center;margin-bottom:20px">
          ...
          <div class="table-product table-product--editable" style="min-width:0"><div><h3 style="margin:0 0 4px">{{g.name}}</h3><small style="color:var(--muted)">Periodo atual</small></div><button class="row-action row-action--edit" title="Editar meta" @click.stop="editGoal(g)"><UiIcon name="edit" :size="15" /></button></div>
        ...
        <small style="display:block;color:var(--muted);margin-top:8px">{{(g.current/g.target*100).toFixed(0)}}% concluido</small>
      </PanelCard>
    </div>
  </div>
</template>
