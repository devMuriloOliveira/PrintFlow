...
const save = () => {
  if (!validate()) return
  goals.value.unshift({ name: form.name, current: 0, target: form.target, color: selected.value.color, icon: selected.value.icon })
  ...
  <div>
    ...
    <div class="split-layout" style="grid-template-columns:minmax(0,1fr) 330px">
      <form @submit.prevent="save">
        ...
        <div class="form-actions"><button class="btn" type="button" @click="cancel">Cancelar</button><button class="btn btn--primary" type="submit">Criar Meta</button></div>
      </form>
