...
const removeMarketplace = async (marketplace: any) => {
  ...
  notify('Marketplace excluido com sucesso.')
}
</script>
...
<template>
  <div>
    ...
    <div class="split-layout" style="grid-template-columns:minmax(0,1fr) 330px">
      <div>
        <div class="metrics-grid metrics-grid--4"><MetricCard label="Canais Cadastrados" :value="formatNumber(marketplaces.length)" icon="store" :change="`${metrics.activeMarketplaces.value} ativos`" /><MetricCard label="Taxa Media" :value="metrics.percent(metrics.marketplaceAverageFee.value)" icon="percent" change="Sobre o valor bruto" color="green" /><MetricCard label="Maior Receita Liquida" :value="formatCurrency(metrics.bestMarketplace.value?.net || 0)" icon="trend" :change="metrics.bestMarketplace.value?.name || '-'" color="green" /><MetricCard label="Maior Taxa" :value="metrics.percent((metrics.highestFeeMarketplace.value?.commission || 0) + (metrics.highestFeeMarketplace.value?.financial || 0) + (metrics.highestFeeMarketplace.value?.ads || 0) + (metrics.highestFeeMarketplace.value?.others || 0))" icon="percent" :change="metrics.highestFeeMarketplace.value?.name || '-'" color="orange" /></div>
        <PanelCard>
          ...
          <div class="table-scroll">
            <table class="data-table">
              <thead><tr><th>Marketplace</th><th>Conexao</th><th>Comissao</th><th>Tarifa Fixa</th><th>Taxa Financeira</th><th>Custo Anuncio</th><th>Outras Tarifas</th><th>Receita Bruta</th><th>Receita Liquida</th><th>Pedidos</th><th>Status</th><th></th></tr></thead>
              <tbody>
      ...
      <aside>
        <PanelCard title="Simulacao de taxas" subtitle="Veja o impacto das taxas em uma venda simulada."><div class="field"><label>Valor da venda</label><input v-model.number="saleValue" type="number"></div><div class="field" style="margin-top:10px"><label>Selecionar marketplace</label><select v-model="selectedName"><option v-for="m in marketplaces" :key="m.name">{{m.name}}</option></select></div><div class="detail-list" style="margin-top:8px"><div class="detail-list__row"><span>Comissao ({{selected.commission}}%)</span><strong>- {{formatCurrency(fees.commission)}}</strong></div><div class="detail-list__row"><span>Tarifa fixa</span><strong>- {{formatCurrency(fees.fixed)}}</strong></div><div class="detail-list__row"><span>Taxa financeira</span><strong>- {{formatCurrency(fees.financial)}}</strong></div><div class="detail-list__row"><span>Custo de anuncio</span><strong>- {{formatCurrency(fees.ads)}}</strong></div><div class="detail-list__row"><span>Outras tarifas</span><strong>- {{formatCurrency(fees.others)}}</strong></div></div><div class="summary-box"><small>Receita Liquida</small><strong class="money-positive" style="display:block;font-size:23px;margin-top:6px">{{formatCurrency(net)}}</strong><span class="badge badge--green" style="margin-top:7px">{{(net/saleValue*100).toFixed(1)}}% do bruto</span></div><button class="btn btn--wide" style="margin-top:12px" @click="notify('Comparacao calculada com os canais ativos')">Comparar marketplaces</button></PanelCard>
