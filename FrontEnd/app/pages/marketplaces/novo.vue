...
const save = async () => {
  ...
  saving.value = true
  try {
    await createItem('marketplaces', { name: form.name, short: (form.short || form.name[0]).slice(0, 2), color: form.color, commission: form.commission, fixed: form.fixed, financial: form.financial, ads: form.ads, others: form.others, active: form.active })
  ...
  <div>
    ...
    <div class="split-layout" style="grid-template-columns:minmax(0,1fr) 330px">
      <form @submit.prevent="save">
        ...
        <div class="form-actions"><button class="btn" type="button" @click="cancel">Cancelar</button><button class="btn btn--primary" type="submit" :disabled="saving">{{ saving ? 'Salvando...' : 'Salvar Marketplace' }}</button></div>
      </form>
