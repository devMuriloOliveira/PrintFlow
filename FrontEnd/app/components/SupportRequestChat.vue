  ...
  <div v-if="requestId" class="audit-chat" :class="{ 'audit-chat--open': open }">
    ...
    <section v-if="open" class="audit-chat__panel">
      ...
      <div ref="messagesElement" class="audit-chat__messages" aria-live="polite">
        ...
        <article v-for="message in messages" :key="message.id" class="chat-message" :class="message.senderType === 'support' ? 'chat-message--incoming' : 'chat-message--outgoing'">
...
/* O painel nao cresce com o historico: somente a lista de mensagens rola. */
.audit-chat__panel{display:flex;height:min(620px,calc(100dvh - 94px));max-height:none;flex-direction:column}
.audit-chat__messages{min-height:0;max-height:none;flex:1 1 auto}
.audit-chat__composer,.info-note{flex:0 0 auto}

/* Cada pessoa ve as proprias mensagens a direita, como em apps de conversa. */
.chat-message--incoming{align-self:flex-start;flex-direction:row}
.chat-message--outgoing{align-self:flex-end;flex-direction:row-reverse}
.chat-message--outgoing .chat-message__avatar{border-color:#1f65dc;color:#fff;background:var(--blue)}
.chat-message--outgoing .chat-message__content{border-color:transparent;border-radius:14px 14px 4px 14px;color:#fff;background:linear-gradient(135deg,var(--blue),var(--blue-dark));box-shadow:0 5px 15px rgba(23,104,242,.2)}
.chat-message--outgoing .chat-message__content small{color:#dceaff}
</style>
