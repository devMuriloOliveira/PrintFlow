...
const makePoints = (values: number[]) => {
  ...
  return values.map((p, i) => `${40 + (i / steps) * 1020},${190 - ((p - min) / range) * 145}`).join(' ')
}
const points = computed(() => makePoints(props.values))
const points2 = computed(() => props.second ? makePoints(props.second) : '')
const pointX = (index: number) => 40 + (index / Math.max(props.values.length - 1, 1)) * 1020
</script>
<template>
  <div class="line-chart">
    <svg viewBox="0 0 1100 230" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Grafico de linhas">
      <g class="chart-grid"><line v-for="y in [45,80,115,150,185]" :key="y" x1="40" :y1="y" x2="1060" :y2="y" /></g>
      ...
      <g class="chart-labels">
        <text v-for="(label, i) in labels" :key="label" :x="40 + (i / Math.max(labels.length - 1, 1)) * 1020" y="218" text-anchor="middle">{{ label }}</text>
      </g>
    </svg>
  </div>
</template>
