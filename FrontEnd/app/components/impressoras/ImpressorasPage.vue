  ...
  <div>
    ...
    <div class="split-layout" style="grid-template-columns:minmax(0,1fr) 390px">
      ...
      <aside class="detail-card">
        <div v-if="nextJobNeedingGcode && selected?.agentId && selected?.agentPrinterId" class="summary-box" style="margin-bottom:12px"><strong>Arquivo 3MF aguardando preparo</strong><p style="margin:5px 0;color:var(--muted);font-size:10px">O OrcaSlicer gerará o G-code do próximo item sem iniciar a impressora.</p><button type="button" class="btn" :disabled="queueLoadingId !== ''" @click="prepareNextGcode">Preparar próximo G-code</button></div>
