...
const {
  session, tenants, tenantPage, tenantPageSize, requests, authorizedTenantAudit, error, activeRequests,
  statusClass, statusLabel, load, loadTenants, refreshTenants, platformPlans, loadPlatformPlans, updatePlatformPlanBillingConfiguration
...
const billingConfiguration = ref<{ plan: any; monthlyReferencePrice: number; yearlyReferencePrice: number; reason: string } | null>(null)
const hasNextTenantPage = computed(() => tenants.value.length === tenantPageSize)
const changeTenantPage = async (delta: number) => {
  const next = Math.max(0, tenantPage.value + delta)
  if (next === tenantPage.value || (delta > 0 && !hasNextTenantPage.value)) return
  await loadTenants(true, next)
}
...
const subscriptionSummary = computed(() => {
  ...
  return [
    ...
    { label: 'Trial histórico', value: tenants.value.filter(tenant => tenant.subscriptionStatus === 'trial').length, className: 'status-pill--under-review' },
...
const openBillingConfiguration = (plan: any) => {
  actionError.value = ''
  billingConfiguration.value = { plan, monthlyReferencePrice: Number(plan.monthlyReferencePrice || 0), yearlyReferencePrice: Number(plan.yearlyReferencePrice || 0), reason: '' }
}
