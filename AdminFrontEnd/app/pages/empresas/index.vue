...
const billingConfiguration = ref<{ plan: any; monthlyPlanId: string; yearlyPlanId: string; trialDays: number; reason: string } | null>(null)
...
const openBillingConfiguration = (plan: any) => {
  actionError.value = ''
  billingConfiguration.value = { plan, monthlyPlanId: plan.mercadoPagoMonthlyPlanId || '', yearlyPlanId: plan.mercadoPagoYearlyPlanId || '', trialDays: Number(plan.trialDays || 7), reason: '' }
}
...
const saveBillingConfiguration = async () => {
  ...
  try {
    await updatePlatformPlanBillingConfiguration(config.plan.id, {
      mercadoPagoMonthlyPlanId: config.monthlyPlanId,
      mercadoPagoYearlyPlanId: config.yearlyPlanId,
...
<template>
  <AdminShell v-model:search="search" title="Empresas" subtitle="Gerencie os tenants da plataforma" :request-count="activeRequests.length">
    ...
    <section class="panel company-filters"><div><strong>Planos e teste gratuito</strong><small>Configure os IDs dos dois planos do Mercado Pago. Esses IDs nao sao exibidos aos clientes.</small></div><button class="button button--quiet" @click="loadPlatformPlans">Atualizar planos</button><button v-for="plan in platformPlans.filter(item => item.active)" :key="plan.id" class="button button--primary" @click="openBillingConfiguration(plan)">Configurar {{ plan.name }}</button></section>
    ...
    <section v-if="billingConfiguration" class="access-modal"><div class="modal-card"><button class="modal-close" :disabled="actionLoading" @click="billingConfiguration = null">Cancelar</button><span class="section-kicker">Cobranca auditada</span><h2>{{ billingConfiguration.plan.name }}</h2><p>Os planos devem ter o mesmo teste gratuito configurado no Mercado Pago. A assinatura e o acesso so sao ativados por webhook valido.</p><form @submit.prevent="saveBillingConfiguration"><label>ID do plano mensal Mercado Pago<input v-model.trim="billingConfiguration.monthlyPlanId" maxlength="160" required autocomplete="off"></label><label>ID do plano anual Mercado Pago<input v-model.trim="billingConfiguration.yearlyPlanId" maxlength="160" required autocomplete="off"></label><label>Dias de teste<input v-model.number="billingConfiguration.trialDays" type="number" min="0" max="30" required></label><label>Motivo da configuracao<textarea v-model="billingConfiguration.reason" minlength="8" maxlength="500" required></textarea></label><button class="button button--primary" :disabled="actionLoading || billingConfiguration.reason.trim().length < 8">{{ actionLoading ? 'Salvando...' : 'Salvar configuracao' }}</button></form></div></section>
  </AdminShell>
</template>
