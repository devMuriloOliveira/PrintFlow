...
const {
  session, requests, messagesByRequest, authorizedTenantAudit, error, activeRequests, closedRequests, tenantFor, formatDate,
  statusLabel, statusClass, isChatOpen, load, loadMessages, refreshRequests, loadChatAssignees, claimChat, transferChat, addChatCollaborator
...
const filteredRequests = computed(() => {
  ...
  return requests.value.filter(request => `${request.id} ${request.subject} ${request.reason} ${request.category} ${request.requesterName} ${tenantFor(request.tenantId)?.name || request.tenantId}`.toLowerCase().includes(term))
})
...
const transferSelectedChat = async () => {
  ...
  actionLoading.value = true; actionError.value = ''
  try { await transferChat(selectedRequest.value.id, assignmentTarget.value); selectedRequest.value = null } catch (cause: any) { actionError.value = cause?.data?.error || cause?.message || 'Nao foi possivel transferir a conversa.' } finally { actionLoading.value = false }
}
