...
const refreshSelected = async () => {
  ...
  if (!requestId) return
  await Promise.all([refreshRequests({ search: search.value.trim(), category: categoryFilter.value, assigneeId: assigneeFilter.value, tenantId: tenantFilter.value, from: fromFilter.value, to: toFilter.value, limit: String(requestPageSize), offset: String(requestPage.value * requestPageSize) }), loadMessages(requestId, true)])
